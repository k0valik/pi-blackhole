...
| `retainedToolOutputMaxTokens` | `20000` | Dedicated budget applied to the retained tail at each Blackhole compaction. Newest consumed tool-output text is retained first; older or oversized text becomes compact `recall #N` markers. The persisted representation stays fixed until the next compaction, raw history remains unchanged, and pending results and non-text parts remain visible. |
