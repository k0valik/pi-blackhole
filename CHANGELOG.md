...
- **Legacy estimate escape hatch (deprecated).** `PI_BLACKHOLE_LEGACY_ESTIMATE=1` forces trigger counters back to chars/4 so pre-0.5.0 pinned-threshold setups keep their exact old cadence while migrating. Env-only and scheduled for removal after 1–2 minors; `/blackhole memory` tags legacy mode. ([plan-08 D15](../work_docs/plan-08-live-validation.md))
- **Peak-inferred session window floor.** When the branch shows the active model already served prompts larger than its resolved context window (missing/wrong registry entries — local llama.cpp models especially), derived thresholds rise to stay at-or-above observed reality: a 1M-window model with no registry entry no longer compacts at ~83k. Peaks are scoped to the branch-tail assistant message's model, so switches are tracked from the first post-switch turn (`setModel` applies immediately while an in-flight loop keeps its stream). Debug event: `session_window.inferred`.
- **Branch-aware session tooling.** New `scripts/om-session-parser.mjs` (active-branch ancestor walks, om coverage-marker extraction across all four schemas, compaction provenance) and a rewritten `scripts/analyze-token-estimation.mjs`: event-based windows with reason-coded exclusion of degenerate pairs, plus a measurement-decomposition section separating tokenizer density from scope overhead. The flat "~1.45×" migration multiplier is retired as content/provider-dependent.
...
- **Env overrides accept `0`.** `PI_BLACKHOLE_COMPACT_AFTER_TOKENS`, `PI_BLACKHOLE_OBSERVE_AFTER_TOKENS`, `PI_BLACKHOLE_REFLECT_AFTER_TOKENS`, `PI_BLACKHOLE_OBSERVATIONS_POOL_MAX_TOKENS`, `PI_BLACKHOLE_OBSERVATIONS_POOL_TARGET_TOKENS`, `PI_BLACKHOLE_REFLECTOR_INPUT_MAX_TOKENS`, `PI_BLACKHOLE_DROPPER_INPUT_MAX_TOKENS`, `PI_BLACKHOLE_OBSERVER_CHUNK_MAX_TOKENS`, `PI_BLACKHOLE_OBSERVER_PREAMBLE_MAX_TOKENS`, and `PI_BLACKHOLE_PROVIDER_IDLE_TIMEOUT_MS` now accept non-negative integers (`0` = auto-derive/disabled). New `PI_BLACKHOLE_THRESHOLD_SCALE` (float, `[0.1, 10]`).
