...
- **Review follow-ups (PR #107)**. CodeRabbit/kilo review of this branch caught real issues, all fixed: (1) `mergeFileLines` split entries on every comma, breaking the combined `(staged,unstaged)` git tag into partial keys that defeated prev/fresh dedup and duplicated the file — entries now split on top-level commas only. (2) `extractHeredocMessage` matched the first heredoc in the whole bash call, so an earlier command's heredoc (e.g. `cat > notes.md <<'EOF'`) became the commit message and outranked the real message — the search now starts at the commit's own `-F -`/`--file`/`--stdin` flag. (3) CJK information-questions matching a standing-instruction marker (`为什么以后要用 pnpm？`, `以后怎么提交代码？`) were retained as preferences — CJK interrogatives (`为什么`/`怎么`/`如何`/…) on `?`/`？`-ending lines are now rejected (anywhere-match, since CJK fronts time-scopes before the interrogative). (4) The two synchronous git calls in `loadGitFileTags` had no timeout despite running in the `session_before_compact` path — now 2s (`rev-parse`) / 5s (`status`), still fail-closed. (5) `filesTouchedCount` ran `collectFilesTouched` on every compaction despite the "zero cost" comment (the `trace` flag applies inside the call, after eager argument evaluation) — now gated on `debugLog`. Debug-script fixes: commit detection recognizes `&&`/`||`/newline boundaries, and the branch-break warning counts `model_change` entries from the raw window (it previously filtered to messages first, so it could never fire). The git-status test resolves its tmpdir (macOS `/var` vs `/private/var`).

- **`[Commits]` follow-up: segment-scoped flags, `--message` long form, root/detached success lines, name-checked pairing** ([#105](https://github.com/k0valik/pi-blackhole/issues/105)). Four gaps in the Bug 2 rewrite, all precision-safe: (1) `-m`/`--message` extraction ran on the whole command string, so an `-m` belonging to a later command (`… && docker run -m 2g img`) or a `-m`-like line inside the heredoc body shadowed the real message — extraction is now scoped to the commit invocation's own token segment (bare newlines split segments; `\`-continuations still join). (2) Space-separated `--message "…"` (the normal long-form spelling) was not recognized, only `--message=…`. (3) Success-line parsing missed first-commit `[main (root-commit) hash]` and `[detached HEAD hash]` forms, in both the subject and the hash recovery. (4) Command/result pairing took the first `tool_result` in the window regardless of tool, so a sibling tool's result (or its error) could misattribute or kill the commit — results whose tool name differs from the call are now skipped. Prior precision guards unchanged.

- **`[User Preferences]` follow-up: bare short corrections and future-scoped CJK instructions** ([#105](https://github.com/k0valik/pi-blackhole/issues/105)). The 5-char floor ran before pattern matching, so the issue's own example correction (`回退`, 2 chars) was still dropped — CJK lines now floor at 2 (all markers are 2+ chars, so no single-char noise is admitted). Added future-scoped standing-instruction markers (`以后`/`下次`/`必须`, preference signals by construction) and `记住` with an acknowledgement guard (`记住了`/`记住吧`/`记住哦` acknowledge rather than direct, and stay excluded). Implicit directives with no marker at all (`只清理残留文件`, `都要修复正确哦`) remain unflaggable without semantic analysis — deliberately left out to avoid false positives.

- **`[Outstanding Context]` follow-up: benign-compound guard for CJK stems** ([#105](https://github.com/k0valik/pi-blackhole/issues/105)). The unguarded stems false-positived on mechanism discussion (`我们需要添加错误处理逻辑` flagged as a blocker). Benign compounds (`错误处理`/`信息`/`消息`/`码`/`类型`/`日志`/`堆栈`, `失败重试`/`失败率`) are stripped before the stem test, so a line flags only on a residual stem — mixed lines (`错误处理逻辑里还是报错`) still fire via the surviving stem. Companion recall fixes: the CJK floor drops 5 → 2 (short reports like `失败了` are complete), and the sentence-start gate accepts `【`/`『`/`「`-led CJK headings. English profile unchanged.

- **`[Files And Changes]` merge no longer drops files when summaries wrap or quote the section name** ([#105](https://github.com/k0valik/pi-blackhole/issues/105), follow-up to the Bug 1 fix). Two merge bugs combined to truncate the section to whatever happened to sit on the first physical line: (1) `sectionOf` matched inline `` `[Files And Changes]` `` mentions inside hand-written pi-native summaries, so the "previous section" became a blob from that mention to the end of the text and prose lines shaped like `- Modified: …` bled in as phantom entries; headers must now start at a line boundary. (2) `formatSummary` wraps long lines at 120 chars before the merge runs, so a fresh `- Modified: a, b, c…` bullet spans several physical lines of which only the first carries the `- Modified: ` prefix — continuation lines are now rejoined before parsing (applies to all merged bullet sections, not just Files). Additionally `extractFiles` canonicalizes Pi `fileOps` seeds and legacy relative tool-arg paths through `cwd`, so `src/a.ts` and `/repo/src/a.ts` no longer coexist as phantom duplicates that defeat display trimming. Reproduced end-to-end on the archived #105 session window (147 messages): `compile()` with the previous summary rendered 1 modified + 1 read before, the full list after. The `before_compact.summary_generated` debug trace now also carries `filesTouchedCount` so the next live compaction can confirm what the in-memory collector saw.

- **`[Commits]` now captures the commit forms a careful developer actually uses** ([#105](https://github.com/k0valik/pi-blackhole/issues/105), Bug 2). The extractor previously required the literal adjacent string `git commit` plus a quoted single-line `-m "…"`, so `git -c user.name=… commit -q -F - <<'MSG'…` — the correct way to commit with an unset identity and a multi-paragraph body — was invisible, and the session's most important artifact survived only by luck in the OM pool. Replaced with a quote-aware token parser: detection matches a `commit` subcommand token preceded by a `git` binary token within the same shell segment (any flag order, line continuations joined, heredoc bodies stripped so a `cat <<EOF` mentioning `git commit` cannot false-positive), and message recovery tries in order: `-m`/`--message` (any position), `-F -`/`--file=-` heredoc subject (comment lines skipped, body not pasted), git's own `[branch hash] subject` success line, then a stable `(message not captured)` placeholder when only a hash is known — and drops the entry entirely when neither is recoverable (quiet `--file` commits). Precision guards: `--dry-run`, failed commits (`nothing to commit` output, tool-result errors, nonzero bash exit) produce nothing.
- **`[User Preferences]` no longer assumes English** ([#105](https://github.com/k0valik/pi-blackhole/issues/105), Bug 3). Added correction-anchored patterns — English (`stop using …`, `that's wrong`, `revert/undo that`) and common CJK directive markers (`不要`/`不用`/`别再`/`回退`/`错了`/`停止`, deliberately excluding broad negatives like `不行`/`不对` that occur in chatter). The blunt `?` rejection is replaced by an interrogative-opener gate: information questions (`What should I use here?`) are still dropped, but directive questions (`Can you always run tests before pushing?`) are now captured.
- **`[Outstanding Context]` scans the whole fresh window instead of the last 20 blocks** ([#105](https://github.com/k0valik/pi-blackhole/issues/105), Bug 4). A blocker raised early in a long session was structurally unreportable, and because the section is volatile (never merged across compactions) it vanished permanently once it scrolled out of the tiny window. Now: full-block scan with case-insensitive dedup (latest wording wins) and the most recent 5 kept; sentence-start gate accepts CJK characters (consistent with brief.ts's `\p{L}` usage); CJK failure stems (`失败`/`报错`/`错误`/`卡住`/`崩溃`) added alongside the unchanged English regex; the 15-char floor drops to 2 for CJK lines (see follow-up entry for the benign-compound guard). English false-positive profile unchanged: capital/quote start still required, continuation fragments and lowercase starts still skipped.
- **`[Files And Changes]` now attributes files the older extractor missed entirely** ([#105](https://github.com/k0valik/pi-blackhole/issues/105), Bug 1). The section previously matched only a hardcoded English/PascalCase tool-name list plus `PATH_KEYS` arg sniffing, so on a modern Pi session (lowercase `read`/`write` tools, anchor-based `replace`/`insert` edit tools that carry no path argument, and file mutations done through bash) it listed wrong or missing files — in the reported session it named ten already-deleted `/tmp` scratch scripts and none of the six delivered files. New `src/extract/file-touch.ts` (ported from the proven pi-files-touched implementation) correlates tool calls with their results and attributes reads/writes/edits/moves/deletes across: native tools via path args, anchor-based edit tools via result-text path recovery (`Successfully replaced in <path>.`, `Successfully wrote N bytes to <path>`, no-op results suppressed), and a bash command parser (redirects, `sed -i`, `tee`, `cp`, `mv` with old→new op redirect, `rm`/`git rm`, `touch`, `patch`, `curl -o`, `cat`/`head`/`tail` reads, heredoc bodies ignored, `/dev/null` and non-literal operands skipped). Files whose most recent operation is a delete no longer appear at all. The collector runs only at compaction time (inside `buildSections`), never per tool call; Pi's own `preparation.fileOps` lists are still seeded in and the legacy name-based block scan remains as fallback for unknown extension tools. Cross-form references merge (relative + absolute, `path:10-40` read slices stripped), and the common-directory-prefix display trim is unchanged. Files are additionally grounded with git working-tree state (porcelain v1 `-z` parsing ported from pi-files-touched): each listed path gets a word tag — `(staged)`, `(unstaged)`, `(staged,unstaged)`, `(new)`, `(renamed)`, `(deleted)`, `(conflicted)` — and a written/edited file git reports untracked is promoted to `Created:` (the category was previously dead). Tags come from one bounded `git status --porcelain=1 -z` snapshot at compaction (fail-closed: no git / not a repo → no annotations); they annotate only the freshly compiled window — tags carried over from a previous summary are stripped as stale point-in-time state, and cross-compaction dedup keys on the stripped path so a re-touched file's fresh tag wins.
