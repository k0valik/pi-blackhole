...
- **Tail behavior control:** `tailBehavior: "pi-default"` keeps Pi's default ~20k token tail visible; `tailBehavior: "minimal"` keeps only the last user message (aggressive pi-vcc cut). Auto-triggered defaults to `"pi-default"`, `/blackhole` defaults to `"minimal"`. ([#14](https://github.com/k0valik/pi-blackhole/pull/14))
...
- **`agentLoopReserve` config key:** configurable reserve (default 8K) for system prompt, tool definitions, and agent loop overhead in the context window check. Added to the configure overlay. ([#14](https://github.com/k0valik/pi-blackhole/pull/14))
