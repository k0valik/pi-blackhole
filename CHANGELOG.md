...
- **Redundant memory keys were merged, and two dead keys were removed.** `dropperPoolFullnessThreshold` merged into `dropperPressureThreshold` (`max(oldPressure, oldFullness)`; the new-data floor is now the constant `0.10`), `dropperInputMaxTokens` merged into `reflectorInputMaxTokens`, and `observationsPoolTargetTokens` / `observerPreambleMaxTokens` (which had no effect) were deleted.
