...
- **Routine worker notifications are now optional.** New `showWorkerNotifications` boolean (default `true`) gates the observer/reflector/dropper progress toasts — `blackhole: reading recent conversation for notes`, `blackhole: saved N notes`, `blackhole: building insights from saved notes`, `blackhole: pruning low-value notes`, and the info-level `blackhole: no new notes` notice. Set it to `false` (settings modal → UI → Worker notifications, `/blackhole configure`, the config file, or `PI_BLACKHOLE_SHOW_WORKER_NOTIFICATIONS`) for quiet sessions. Warnings and errors are unaffected: model fallback/unavailability, context-window skips, no-output warnings, worker failures, compaction notifications, and `/blackhole*` command output stay visible.
...
- **The config surface was rebuilt around outcomes, and several redundant or dead keys were folded away.** Every settings-modal label and description now names the user outcome (notes/insights/pruning, "Take notes every", "Prune memory when") instead of the internal worker, grouped into six sections, with a two-line help contract (what the knob does + what your current value means) and a `key:` legend under each focused field. JSON keys are unchanged; the full mapping is in `docs/CONFIG.md#label--key-mapping`.
- **`compactionEngine` was folded into `compaction`.** `"blackhole"` becomes `"automatic"`, `"pi-default"` becomes `"off"`; `"auto"` is still accepted as an alias. The env var `PI_BLACKHOLE_COMPACTION_ENGINE` is removed.
- **The auto-compaction threshold is now one shape plus an optional floor/ceiling band.** `compactAfterBy` (`preset`/`percent`/`tokens`/`reserve`) selects the shape, its value key (`compactAfterPreset`/`compactAfterRatio`/`compactAfterTokens`/`compactReserveTokens`) supplies the number, and the new `compactAfterMinTokens`/`compactAfterMaxTokens` clamp the result: `effective = clamp(shape(window), min, max)`. `compactAfterRatio` is now a **percent** in `(0, 100]` (it was a fraction). This resolves the 256k-vs-1M tension without a per-model override — e.g. `{ "compactAfterBy": "percent", "compactAfterRatio": 46, "compactAfterMaxTokens": 180000 }` gives ~120k on a 256k window and 180k on a 1M window.
- **Redundant memory keys were merged, and two dead keys were removed.** `dropperPoolFullnessThreshold` merged into `dropperPressureThreshold` (`max(oldPressure, oldFullness)`; the new-data floor is derived from the pressure, `clamp(0.15 × P, 0.02, 0.10)`), `dropperInputMaxTokens` merged into `reflectorInputMaxTokens`, and `observationsPoolTargetTokens` / `observerPreambleMaxTokens` (which had no effect) were deleted.
- **Existing config files are migrated on disk.** A two-phase, verified migration rewrites the global and project configs at startup: it backs up once, writes and verifies the new keys, then deletes the consumed legacy keys, preserving unknown keys and continuing in memory on read-only filesystems. See `docs/MIGRATION-GUIDE.md`.
- **A fresh install scaffolds a small curated starter file** instead of the full defaults; absent keys are still filled from the built-in defaults at read time.

> **Deferred:** decoupling the pruner cadence from `reflectAfterTokens` (plan-09 §3.6). The dropper merge above ships without it — removing the `reflectAfterTokens` gate from the pruner's new-data path is a real cadence change and needs its own observation window. **Accepted divergence:** the dropper merge is exact for the default posture (`P = 0.70` reproduces the old `0.10` floor); a custom floor paired with a custom pressure is now derived from the pressure and smoothed, not preserved exactly.
...
- **The read-only "display all" settings modal keeps the Edit action highlighted.** The pre-focused action lost its highlight as soon as the user scrolled the (non-interactive) field list, hiding the only way into edit mode; in read-only mode scrolling no longer clears the tab/action ring focus (edit mode still returns focus to the fields).
...
- **The config migration can no longer make a config worse than leaving it alone.** The `.bak` is now re-read and verified before any write (and the remove-only path takes one too), an unrecognized value skips only that key and notifies via the UI instead of aborting the whole file, a `0` value is read as the documented "not set" sentinel, and `configVersion` is both the gate and the success stamp — a partial migration is never stamped, so it retries and re-warns. The temp+rename writer preserves the file's permissions and never leaks a temp file.
- **A legacy `compactAfterRatio` fraction can no longer be read as a sub-1% percent.** A fraction (`0.65` = 65%) that escaped the on-disk migration — a read-only file, the settings modal, or a skipped value — used to be divided by 100 a second time, collapsing the threshold to under 1% of the window and compacting on nearly every turn; the loader and resolver now scale it once (`windowPercent`) and warn.
- **The dropper's new-data floor is derived from the surviving pressure** (`clamp(0.15 × P, 0.02, 0.10)`) instead of a flat `0.10`, so a lowered `dropperPressureThreshold` no longer waits longer than it used to before pruning.
- **The settings modal no longer truncates the focused field's help copy**, and its `visibleWhen` conditions resolve from the active tab's rows only — a duplicate key in another tab (global/project) could hide the conditional threshold rows.
- **The cross-scope compaction shape is re-derived from the merged value keys.** A project-layer reserve could previously stamp `compactAfterBy: "reserve"` and silently outrank a global `compactAfterTokens` pin. A project-layer `passive` now also arms the falsy `PI_BLACKHOLE_PASSIVE` undo, and a stale `PI_BLACKHOLE_COMPACTION_ENGINE=pi-default` still keeps blackhole out (with a warning).
