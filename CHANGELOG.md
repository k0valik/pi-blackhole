...
- **Memory workers end a finished run early and honor a prompt-cache preference.** The observer's `record_observations` and the reflector's `record_reflections` gained a `complete` flag: when the final batch is flagged `complete` and nothing was rejected, the host sends `terminate`, so a fully-processed run stops instead of spending the rest of its `agentMaxTurns` budget; an incomplete or fully rejected batch keeps going. A new optional `cacheRetention` (`none` \| `short` \| `long`, unset = pi's own default `short`) is forwarded to all three worker streams so prompt caching can be tuned per install — adapters that do not support a value ignore it. Both knobs live in the settings modal (Worker section) and `PI_BLACKHOLE_CACHE_RETENTION`; all three workers already scope their streams to the consolidation `sessionId`. Doc reordering only — the observer prompt keeps its append-stable prefix and never reintroduces local time ([upstream OM `#89`](https://github.com/elpapi42/pi-observational-memory/pull/89)).
