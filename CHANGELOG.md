...
- **Memory workers end a finished run early and honor a prompt-cache preference.** The observer's `record_observations` and the reflector's `record_reflections` gained an optional `complete` flag: when the final batch is flagged `complete` and that batch rejected nothing, the host sends `terminate`, so a fully-processed run stops instead of spending the rest of its `agentMaxTurns` budget; an incomplete or rejected batch keeps going, and a refused `complete=true` now says so in the receipt so the model corrects or finishes instead of re-sending the same batch. A model that omits the flag loses only the early stop — the batch still records. A new optional `cacheRetention` (`none` \| `short` \| `long`, unset = inherit pi's effective setting: provider default `short`, `PI_CACHE_RETENTION=long` opts in) is forwarded to all three worker streams so prompt caching can be tuned per install — adapters that do not support a value ignore it. Both knobs live in the settings modal (Worker section) and `PI_BLACKHOLE_CACHE_RETENTION`; all three workers already scope their streams to the consolidation `sessionId`. Doc reordering only — the observer prompt keeps its append-stable prefix and never reintroduces local time ([upstream OM `#89`](https://github.com/elpapi42/pi-observational-memory/pull/89)).
