...
> **Last verified:** 2026-09-15 against repo rev `83ec994` (`fix/92-headless-inline`) and pi `@earendil-works/pi-coding-agent` 0.85.1. House rule: any claim of the form "Pi does not expose X" in `docs/` must cite the pi version it was checked against — re-check the vendored pi type surface before repeating one.
