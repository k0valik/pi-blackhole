...
**Edge cases**: when nothing is recorded, `ObserverEmptyReason` distinguishes `no_new_content`, `tool_not_called`, `all_rejected` (every `sourceEntryIds` invalid), `all_duplicates`, and `empty_array`. An empty batch flagged `complete: true` is the sanctioned "chunk covered, nothing new" close, so it reports `no_new_content` (info level) rather than the `empty_array` warning an unflagged empty batch earns. If the stream ends with `stopReason: "error"` **and** zero observations were collected, `runObserver` throws so the pipeline can fall back to another model; if some observations were collected, they are kept.
