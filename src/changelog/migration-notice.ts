 ...
 *
 * The config-surface reorg (`work_docs/plan-09`, `plan-10`) rewrites users'
 * on-disk config files at startup. This module tells each user exactly once
 * that their config was migrated, to review their settings, and where to read
 * the upgrade guide.
 *
 ...
 * package version is exactly `MIGRATION_NOTICE_VERSION` and a migration ran
 * this session. Returns true when a notification was attempted. The guard is
 * set even if the sink throws — a stale extension context must never cause a
 * retry nag.
 */
